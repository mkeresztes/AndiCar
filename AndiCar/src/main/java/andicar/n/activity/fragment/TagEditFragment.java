...
public class TagEditFragment extends BaseEditFragment {
    ...
    private void loadDataFromDB() {
        Cursor c = mDbAdapter.fetchRecord(DBAdapter.TABLE_NAME_TAG, DBAdapter.COL_LIST_TAG_TABLE, mRowId);
    ...
    protected boolean saveData() {
        ...
        int dbRetVal;
        if (mRowId == -1) {
            dbRetVal = ((Long) mDbAdapter.createRecord(DBAdapter.TABLE_NAME_TAG, data)).intValue();
            if (dbRetVal > 0) {
                return true;
            } else {
            ...
            int updResult = mDbAdapter.updateRecord(DBAdapter.TABLE_NAME_TAG, mRowId, data);
            if (updResult != -1) {
                String errMsg;
